...
export const Thirteen = () => {
    ...
    switch (value) {
      ...
      case 2:
        return (
          <Tab2
            changeTab={changeTab}
            resetPlayerScoreArray={resetPlayerScoreArray}
          />
        );
      case 3:
        return (
  ...
  const createPlayerArray = () => {
    Array.from({ length: players }).map((_item, index) => {
      return setPlayerScoreArray((playerScoreArray) => [
        ...playerScoreArray,
        { name: `Player ${index + 1}`, score: 0, newscore: 0, index: [index] },
      ]);
    });
  };
      ...
      <playerContext.Provider value={players}>
        <gameContext.Provider value={game}>
          <tabContext.Provider value={tab}>
            <div className="h-screen bg-emerald-900 relative overflow-y-hidden">
              {tabChange(tab)}
              <div
                className={`h-full w-full bg-emerald-900 absolute transition-all ease-in-out duration-300 ${
                  showAlert ? "opacity-100 top-0" : "opacity-0 top-full"
                }`}
              >
                <div
                  className={`h-fit w-3/4 p-4 border-2 text-slate-800 border-gray-400 transition-all ease-in duration-300 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-gray-100 ${
                    showAlert ? "opacity-80" : "opacity-0"
                  } lg:w-1/3 lg:mx-auto`}
                >
                  <p>{`Warning  - you have selected ${players} players. You need ONE deck of cards for every 4 players. Do you have enough?`}</p>
                  <div className="flex justify-between mt-4">
                    <button
                      className="bg-gray-100 border-2 border-gray-200 p-2 text-black hover:bg-gray-200"
